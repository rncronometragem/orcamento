...
$(document).ready(function() {
  $('.cnpj').on('blur', function() {
    var cnpj = $(this).val().replace(/\D/g, ''); // Remove pontos e traço do cnpj
    if (cnpj.length != 11 || !/^\d+$/.test(cnpj)) {
      alert('O cnpj digitado é inválido. Por favor, digite um cnpj válido.');
      return false;
    }
    $.post('busca_cnpj.php', {cnpj: cnpj}, function(data) {
      console.log(data); // Exibe a resposta do servidor no console
      if (data.existe) {
        var idc = data.idc; // Recupera o valor do idc do cliente encontrado
        alert('O cnpj já existe no sistema. Você será redirecionado para a página de edição de cliente.');
        ...
        //alert('cnpj não cadastrado ainda...');
      }
    }, 'json');
  });
});
